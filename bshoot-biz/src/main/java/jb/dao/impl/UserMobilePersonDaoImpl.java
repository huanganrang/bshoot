...
public class UserMobilePersonDaoImpl extends BaseDaoImpl<TuserMobilePerson> implements UserMobilePersonDaoI {
    ...
    public List<String> notAttMobilePerson(UserMobilePersonRequest userMobilePersonRequest) {
        ...
        if(userMobilePersonRequest.isRand())
            sort=" ORDER BY rand() ";
        String hql="SELECT * FROM user_mobile_person t1 where t1.is_delete!=1 and t1.user_id=:userId and t1.friend_id!=:userId and t1.friend_id not null and t1.friend_id not in (select att_user_id from user_attention WHERE is_delete!=1 and user_id=:userId) "+sort+" limit :start,:rows";
    ...
    public List<String> noAttMobilePersonPerson(UserMobilePersonRequest userMobilePersonRequest) {
        String hql="select * from user_mobile_person t1 " +
                "where t1.is_delete!=1 and t1.user_id!=:userId and t1.user_id in (select friend_id from user_mobile_person where is_delete!=1 and user_id=:userId and friend_id not null) " +
